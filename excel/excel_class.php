...
function print_bt() {
	...
	for($i = 1; $i < count($cs); $i++) {
		...
		for($j = 0; $j < count($item['args']); $j++)
			if (is_string($item['args'][$j])) $item['args'][$j] = "\"" . $item['args'][$j] . "\"";
		$args = join(",", $item['args']);
		if (isset($item['class'])) $str = sprintf("%s(%d): %s%s%s(%s)", $item['file'], $item['line'], $item['class'], $item['type'], $item['function'], $args);
		else $str = sprintf("%s(%d): %s(%s)", $item['file'], $item['line'], $item['function'], $args);
		echo $str . "<br>\n";
	}
	print "</code>\n";
}
...
class DebugOut {
	...
	function log($message, $priority = ABC_INFO) {
		...
		if ($priority > $this->_currentPriority) {return false;} // Add to loglines array
		return $this->_writeLine($message, $priority, strftime('%b %d %H:%M:%S'));
	}
	function dump($variable, $name) {
		$priority = ABC_VAR_DUMP;
		if ($priority > $this->_currentPriority) {return false;}
  ...
  */
	...
	function _openfile() {
		if (($this->_fp = @fopen($this->_filename, 'a')) == false) {return false;}
		return true;
	}
	function close() {
  ...
  */
	...
	function attach(&$logObserver) {
		if (!is_object($logObserver)) {return false;}
...
class DataProvider {
	...
	function get($offset, $length) {
		if (!$this->isValid()) _die("Data provider is empty.");
		if ($this->_baseOfs + $offset + $length > $this->_size) _die("Invalid offset/length.");
		switch ($this->_type) {
			case DP_FILE_SOURCE :
				{
					if (@fseek($this->_data, $this->_baseOfs + $offset, SEEK_SET) == -1) _die("Failed to seek file position specified by offest.");
	...
	function isDateFormat($val) {
		...
		if (preg_match("/[m|d|y]/i", $this->format[$f_i]) != 0) {
			if (strrpos($this->format[$f_i], '[') != FALSE) {
				...
				if (preg_match("/[m|d|y]/i", $tmp) != 0) return TRUE;
				else return FALSE;
			} else {
				return TRUE;
			}
 ...
 */
	function find_stream($dir, $item_name, $item_num = 0) {
		...
		$curr_name = '';
		if (($dt == 2) || ($dt == 5)) for($i = 0; $i < ($dir->getOrd($item_num * 0x80 + 0x40) + 256 * $dir->getOrd($item_num * 0x80 + 0x41)) / 2 - 1; $i++)
	...
	function parse_worksheet($ws) {
		...
		$vers = ord($ws[1]);
		if (($vers != 0) && ($vers != 2) && ($vers != 4) && ($vers != 8)) return 8;
		if ($vers != 8) {
			$biff_ver = ($ver + 4) / 2;
		} else {
...
/*DBG*/  $this->dbglog->trace("found RK");
					if (($biff_ver < 3)) break;
					if ((ord($ws[$ptr + 2]) + 256 * ord($ws[$ptr + 3])) < 0x0a) return 6;
					$row = ord($ws[$ptr + 4]) + 256 * ord($ws[$ptr + 5]);
					$col = ord($ws[$ptr + 6]) + 256 * ord($ws[$ptr + 7]);
					$xf_i = ord($ws[$ptr + 8]) + 256 * ord($ws[$ptr + 9]);
...
/*DBG*/  $this->dbglog->trace("found LABEL");
					...
					if ((ord($ws[$ptr + 2]) + 256 * ord($ws[$ptr + 3])) < 8) {return 6;}
					$row = ord($ws[$ptr + 4]) + 256 * ord($ws[$ptr + 5]);
					$col = ord($ws[$ptr + 6]) + 256 * ord($ws[$ptr + 7]);
...
echo str_replace("\n","<br>\n", ExcelFont::toString($font,$fonti));*/
	...
	function parse_workbook($f_header, $dp) {
		...
		if ($workbook_length > 0) {
			...
			if (strlen($wb) != $workbook_length) return 6;
			unset($chain);
		} // Unset fat arrays
		...
		$vers = ord($wb[1]);
		if (($vers != 0) && ($vers != 2) && ($vers != 4) && ($vers != 8)) {return 8;}
		if ($vers != 8) $biff_ver = ($ver + 4) / 2;
		else {
			if (strlen($wb) < 12) return 6;
			switch (ord($wb[4]) + 256 * ord($wb[5])) {
				case 0x0500 :
					if (ord($wb[0x0a]) + 256 * ord($wb[0x0b]) < 1994) $biff_ver = 5;
					else {
		...
		while ( (ord($wb[$ptr]) != 0x0a) && ($ptr < $wblen) ) {
			...
			switch ($opcode) {
				...
				case 0x041e :
					...
					if ($fidx < 0x31 || $fidx == 0x31) break;
					elseif ($biff_ver > 7) $this->format[$fidx] = $this->getUnicodeString($wb, $ptr + 6);
				break;
				// FONT 0x31
				case EXCEL_FONT_RID :
					...
					while ( ($sptr < $ptr + 4 + $sbuflen) && ($sptr < strlen($wb)) && ($snum > 0) ) {
						...
						switch (ord($wb[$sptr + 2]) & 0x0c) {
							...
							case 0x0c :
								...
								if ($sptr + 9 + $slen * $char_bytes > $ptr + 4 + $sbuflen) $slen = ($ptr + $sbuflen - $sptr - 5) / $char_bytes;
							...
							case 8 :
								$rt = ord($wb[$sptr + 3]) + 256 * (ord($wb[$sptr + 4]));
								if ($sptr + 5 + $slen * $char_bytes > $ptr + 4 + $sbuflen) $slen = ($ptr + $sbuflen - $sptr - 1) / $char_bytes;
							...
							case 4 :
								...
								if ($sptr + 7 + $slen * $char_bytes > $ptr + 4 + $sbuflen) $slen = ($ptr + $sbuflen - $sptr - 3) / $char_bytes;
								$str = substr($wb, $sptr + 7, $slen * $char_bytes);
								$sptr += $slen * $char_bytes + 7;
							break;
							// Compressed or uncompressed unicode
							case 0 :
								if ($sptr + 3 + $slen * $char_bytes > $ptr + 4 + $sbuflen) $slen = ($ptr + $sbuflen - $sptr + 1) / $char_bytes;
				...
				switch ($data['type']) {
					...
					case 0 :
						...
						if ($exc->sst['unicode'][$ind]) {
							...
							$s = iconv('utf-16le', 'utf-8', $exc->sst['data'][$ind]);
						} else
							$s = $exc->sst['data'][$ind];
