...
// 我分享的朋友
$follows = $db->getAssoc('info', "follow_id={$_SESSION['uid']} group by uid order by moqi desc,total desc,id desc",'uid,uname,total,searchnum,foundnum,moqi');
...
if(!empty($my['follow_id'])){
  ...
  if(!empty($shares)){
    ...
    $shares_follow = $db->getAssoc('info', "follow_id={$shares['uid']} group by uid order by moqi desc,total desc,id desc",'uid,uname,total,searchnum,foundnum,moqi');
  }
}
$class = empty($shares) ? 'lishi3' : 'lishi4';
...
<div class="lishi">
    ...
    <div class="<?php echo $class ?>">
        ...
        <div class="list-box">
            <h1>我的藏宝计划之搜查达人榜<a href="javascript:void(0)" class="faqi_2"><img src="images/again.png" class="again1"></a></h1>
            <div class="list">
                ...
                <?php if(!empty($follows)):$i = 0;foreach ($follows as $follow): 
                    $p20 = $p30 = '';
                    if(++$i == 1){
                      $p20 = ' width="20%"';
                      $p30 = ' width="30%"';
                ...
                ?>
                  <tr>
                    <td<?php echo $p20 ?>><?php if($i <= 3): ?><img src="images/<?php echo $i ?>.png"><?php else:?><?php echo $i ?><?php endif;?></td>
                ...
                <?php endforeach; else:?>
                <tr><td colspan="4">还没有好友参与哦~</td></tr>
                <?php endif; ?>
                </table>
                </div>
            </div>
            <?php if(!empty($shares)): ?>
           	<h1><strong><?php echo $shares['uname'] ?></strong> 的藏宝计划之搜查达人榜</h1>
            <div class="list">
                <table width="100%" border="0">
                  <tr>
                    <th width="20%">排名</th>
