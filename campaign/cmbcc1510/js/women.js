$(function(){
	...
	$(".numberbox").keyup(function(){
		...
		var v = parseInt($(this).val()), 
			other_total = changed_total = 0,
			other_obj = $(this).parent().siblings(".check"),
			changenum = other_obj.length,
			vals = [];
		if(isNaN(v)) return;
		if(v <= 0) v = 1;
		if(v >= 40) v = 40;
		...
		if(changenum == 0){
			var temp = $(this).parent().siblings('.check:first'),
				tempipt = temp.children('input'),
				tempv = parseInt(tempipt.val()),
				left = _max - changed_total - v;
			if((tempv + left) < 1){
				tempipt.val(1);
				var temp_total = 0;
				other_obj.each(function() {
					temp_total += parseInt($(this).children('input').val())
				});
				$(this).val(_max-temp_total);
				return;
			}
			tempipt.val(tempv + left);
			return;
		}

		if(changenum == 2){
			var other_vals = _max - v;
			if(other_vals % 2 == 0){
				vals.push(other_vals/2);
				vals.push(other_vals/2);
			}else{
				var tempv = Math.floor(other_vals/2);
				vals.push(tempv);
				vals.push(_max - tempv - v);
			}
		}else{
			var temp = _max - changed_total - v;
			if(temp < 1){
				other_obj.each(function() {
					if(!$(this).hasClass('changed')){
				...
				$(this).val(_max-changed_total-1);
				return;
			}
			vals.push(_max - changed_total - v);
		}
		
		other_obj.each(function(index, el) {
			if(!$(this).hasClass('changed')){
				$(this).children('input').val(vals.pop());
			}
		});
		return;*/
