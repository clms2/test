<?php 
require 'inc/check.php';
require 'inc/conn.php';
require 'inc/func.php';
require 'inc/CMB.class.php';

// 假如不是从index跳过来的就再判断下是否关注  header location过来的没有referer？？。。
// $refer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
if(!DEBUG){
    if(empty($refer) || stripos($refer, 'index.php') === false){
        $bindinfo = CMB::getBindInfo($_SESSION['openid']);
        $_SESSION['isbind'] = $bindinfo['isBind'] == 2 ? true : false;
        // 未关注,在点击开始寻宝的时候跳转
        if($bindinfo['isBind'] === '0'){
            $_SESSION['need_guanzhu'] = 1;
        }else{
            unset($_SESSION['need_guanzhu']);
        }
    }
}

// 分享给我的人的好友
$shares = $db->getOneAssoc('info', "uid={$_SESSION['follow_id']}",'uid,uname,total,searchnum,foundnum,moqi');
$shares_follow = $db->getAssoc('info', "follow_id={$_SESSION['follow_id']} group by uid order by moqi desc,total desc,id desc",'uid,uname,total,searchnum,foundnum,moqi');

?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="keywords" content="" />
<meta name="description" content="" />
<meta name="viewport" content="width=device-width,initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no" /> 
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
<meta name="format-detection" content="telephone=no"/>
<title>70万神秘海底宝藏</title>
<link rel="stylesheet" href="css/css.css"/>
<link rel="stylesheet" href="css/animation.css"/>
<?php if (isset($_SESSION['need_guanzhu'])): ?>
<script type="text/javascript">
    var need_guanzhu = '<?php  echo GUANZHU?>';
</script>
<?php endif; ?>
</head>
<body style="max-width:640px">
<div class="guide-friend">
	<img src="images/jijiehao.png" class="jijiehao">
    <img src="images/friend-text.png" class=" guide-text">
	<div class="shuoming"></div>
    <div class="userid"><?php echo $shares['uname'] ?></div>
    <div class="kaishixunbao"><img src="images/pop-form-btn.png" style="width:100%"></div>
    <div class="guide-bottom"><img src="images/guide-bottom.png" style="height:100%; vertical-align:bottom">
    	<div class="list-box">
        	<h1><strong><?php echo $shares['uname'] ?></strong>  的藏保计划排行榜</h1>
            <div class="list">
                <table width="100%" border="0">
                  <tr>
                    <th width="15%">排名</th>
                    <th width="35%">昵称</th>
                    <th width="30%">次数/保额</th>
                    <th width="20%">默契度</th>
                  </tr>
                </table>
                <div>
                <table width="100%" border="0" style="margin:0; padding:0">
                <?php if(!empty($shares_follow)):$i = 0;foreach ($shares_follow as $follow): 
                    $p20 = $p30 = '';
                    if(++$i == 1){
                      $p20 = ' width="20%"';
                      $p30 = ' width="30%"';
                    }
                ?>
                  <tr>
                    <td<?php echo $p20 ?>><?php if($i <= 3): ?><img src="images/<?php echo $i ?>.png"><?php else:?><?php echo $i ?><?php endif;?></td>
                    <td<?php echo $p30 ?>><?php echo $follow['uname'] ?></td>
                    <td<?php echo $p30 ?>><?php echo $follow['searchnum'] ?>次/<?php echo $follow['total'] ?>万</td>
                    <td<?php echo $p20 ?>><?php echo $follow['moqi'] ?>%</td>
                  </tr>
                <?php endforeach; else:?>
                <tr><td colspan="4">他还没有好友参与哦~</td></tr>
                <?php endif; ?>
                </table>
                </div>
            </div>
    </div>
</div>
<div class="fix"></div>

<!--规则介绍-->
<?php include 'rule.html'; ?>
<!--规则介绍-->

<!--设置藏保-->
<?php include 'setplace.html'; ?>
<!--设置藏保-->

<!--分享-->
<?php include 'share.html' ?>
<!--分享-->

<script src="js/jquery-1.11.1.min.js"></script> 
<script src="/js/jweixin-1.0.0.js"></script>
<script src="js/women.js?v=110"></script> 

</div>
</body>
</html>
